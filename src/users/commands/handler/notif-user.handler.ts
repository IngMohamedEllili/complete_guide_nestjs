...
export class UserNotificationHandler implements ICommandHandler<UserNotificationCommand>{
  ...
  async execute(command: UserNotificationCommand){
    Logger.log('async NotifiedUser', 'NotifiedUser')
      const { user }= command
      console.log(user)
      this._publisher.publish(new NotifiedUserEvent(user),new NotifiedUserEvent(user).streamName)
      return user
  }
}